 ...
 *
 * 汇集所有能力在浏览器环境的实现：纯计算能力两端共用同一实现，
 * 平台特定能力（如 image.resize）在此注册其 Canvas 版本。
 * 新增工具时在此追加一行 register 即可，无需改动 core。
 *
 * @returns 已注册全部浏览器侧能力的注册表
 */
function createBrowserRegistry(): CapabilityRegistry {
